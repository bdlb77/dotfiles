...
| Secrets        | [Varlock](https://varlock.dev/) + [1Password](https://1password.com/) | Schema-driven env vars, vault-backed |
...
├── zed/                   # NOT stowed; rendered via `op inject` from settings.tpl.json
├── vscode/                # symlinked manually (lives outside $HOME)
├── ruby/                  # → ~/.irbrc, ~/.rspec, ~/.gemrc
├── secrets/               # Varlock schema for 1Password-backed env vars
├── scripts/               # helpers (render-zed.sh, ...)
...
### `zed/` + `secrets/` — secrets via Varlock + 1Password

Zed's `settings.json` contains MCP server tokens (GitHub PAT, Figma API key,
etc.) — these can't be committed in plaintext. Pattern:

1. **Committed template** (`zed/.config/zed/settings.tpl.json`) — contains
   `${VAR}` placeholders, **not** real values
2. **Runtime file** (`~/.config/zed/settings.json`) — gitignored, generated by
   `scripts/render-zed.sh` (which uses `varlock run -- envsubst` to resolve
   secrets from the 1Password Environment and inject them)
3. **Re-render after editing the template**: `make render-zed`

For shell env vars, use Varlock:
- Add to `secrets/.env.schema` with `op(op://...)` references
- `varlock run -- some-command` injects vars for one invocation
- **Auto-loaded into every interactive zsh** via `scripts/load-secrets.zsh`,
  which `~/.zshrc.local` sources (Touch ID prompts once per session)
- See `secrets/README.md` for full details

A **pre-commit hook** (`scripts/pre-commit.sh`, installed via `make install-hook`)
runs `varlock scan --staged` before every commit and blocks any commit that
contains a plaintext copy of a resolved sensitive value. Bypass with
`git commit --no-verify` (emergencies only).

**One-time setup per machine:**
1. Install 1Password desktop app + sign in
2. 1Password → **Settings → Developer** → enable "Integrate with 1Password CLI"
3. `op signin` to verify
4. `make render-zed`
